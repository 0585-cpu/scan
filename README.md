...
For the current implementation, verification, release, and `UNVERIFIED` status, see [docs/current-development-status-ko.md](docs/current-development-status-ko.md). For how the scan paths are measured and tested, see [docs/testing-and-measurement-ko.md](docs/testing-and-measurement-ko.md). For a portable release artifact, see [docs/install.md](docs/install.md). For day-to-day usage, see [docs/user-guide.md](docs/user-guide.md). Windows artifacts use `.zip`; macOS/Linux artifacts use `.tar.gz`; each artifact is written with a `.sha256` checksum.
