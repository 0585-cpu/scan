...
`scaprobe` is an authorization-first network diagnostics toolkit that combines:

- High-throughput TCP connect and UDP response-based scanning through the required `scaprobe-engine`
...
On another Windows x64 PC, extract the release ZIP and run `Start-Scaprobe.cmd`. Its menu can start Scaprobe, install screenshot support and start, install or update only, or run diagnostics. Normal start performs first-run setup automatically. For automation, use `Start-Scaprobe.cmd --start`, `--screenshots`, `--setup`, or `--diagnostics`. The lower-level `bin\setup.cmd` and `bin\start-desktop.cmd` scripts remain available for separate setup and launch steps. The destination PC needs Python 3.10+ and internet access during setup; TCP/UDP scanning itself does not require Npcap.
...
cargo +stable-x86_64-pc-windows-gnu build --profile portable -p scaprobe-engine
```
...
```powershell
$env:CARGO_TARGET_DIR = Join-Path $env:TEMP 'scaprobe-cargo-target'
cargo +stable-x86_64-pc-windows-gnu build --profile portable -p scaprobe-engine
```
...
Start Scaprobe as a local desktop-style app:

```powershell
scaprobe desktop
```

This starts the local API/dashboard on the fixed `127.0.0.1:8765` address and opens the dashboard. Pass `--port 0` only when an automatically selected free port is preferred. Use `scaprobe desktop --no-open` when a desktop wrapper or shortcut will open the URL itself.
...
```powershell
scaprobe scan --targets 192.168.1.0/28 --ports 1-1024 --scope 192.168.1.0/24 --json --confirm-authorized
```

Read targets and ports from files, exclude a host, and include a named profile:

```powershell
scaprobe scan --targets-file .\targets.txt --ports-file .\ports.txt --exclude 192.168.1.50/32 --profile web --scope 192.168.1.0/24 --confirm-authorized
```

Use built-in common ports:

```powershell
scaprobe scan --targets 192.168.1.10 --top-ports 20 --scope 192.168.1.0/24 --confirm-authorized
```

Use a configuration file for scan defaults, custom port profiles, excludes, scope, and environments:

```powershell
scaprobe scan --config .\scaprobe.toml --env lab --targets 192.168.1.10 --profile web-plus --confirm-authorized
scaprobe serve --config .\scaprobe.toml --env corp
```

If `--config` is omitted, Scaprobe checks `.\scaprobe.toml` and then the user config directory. Example config: `docs/scaprobe.example.toml`.
...
Plugins can also be listed in `scaprobe.toml` under `[plugins].paths`. Plugin manifests are JSON data only; Scaprobe does not execute plugin code. Python validates and merges runtime fingerprint rules, then passes a temporary schema-version-1 catalog to the Rust engine. Port profiles remain Python-side input presets.
...
```powershell
scaprobe scan --targets 192.168.1.0/24 --ports 1-1024 --max-attempts 100000 --confirm-large-scan --scope 192.168.1.0/24 --confirm-authorized
```
...
```powershell
scaprobe export <scan_id> --format json
scaprobe export <scan_id> --format csv --state open --output .\open-ports.csv
scaprobe export <scan_id> --format ndjson --protocol udp --output .\udp-results.ndjson
```
...
scaprobe report <scan_id> --format markdown --output .\scan-report.md
```
...
```powershell
scaprobe capture --output .\capture.pcap --duration-s 10 --filter "tcp port 80" --confirm-authorized
...
Start `scaprobe serve`, then use the dashboard or `/v1/oast/sessions` REST endpoints to create HTTP callback tokens and inspect interactions. OAST support records inbound HTTP metadata only; it does not generate exploit payloads or perform vulnerability checks.
...
scaprobe send udp --target 192.168.1.20 --dport 9999 --payload-text "hello" --scope 192.168.1.0/24 --confirm-authorized
...
A loopback bind (`127.0.0.1`, `localhost`, `::1`) serves the API without a token, as before. Any other bind is reachable from the network, so the API is never served unauthenticated: supply `--api-token` or `SCAPROBE_API_TOKEN`, and one is generated and printed at startup when you do not.
...
```powershell
scaprobe serve --host 0.0.0.0 --port 8765 --api-token (python -c "import secrets;print(secrets.token_urlsafe(32))")
```
...
```powershell
scaprobe diagnostics
```

Import `postman/scaprobe.postman_collection.json` into Postman and set `baseUrl` if needed.
...
{
  "status": "ok",
  "db": "C:\\Users\\you\\AppData\\Roaming\\Scaprobe\\scaprobe.db",
  "diagnostics": {
    "app_version": "0.1.0",
    "rust_engine_available": true,
    "rust_engine_version": "scaprobe-engine 0.1.0",
