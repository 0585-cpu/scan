...
이 문서는 Netroach 소스 코드를 다른 PC로 옮겨 개발을 이어가기 위한 환경·이전 가이드다. 이 문서의 상세 수치와 예시는 `0.1.0` 시점의 기록을 포함한다. `0.2.1`의 구현·검증·릴리즈 현황과 최신 `UNVERIFIED` 항목은 [현재 개발 현황](current-development-status-ko.md)을 기준으로 한다.
