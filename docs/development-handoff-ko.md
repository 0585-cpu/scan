# Scaprobe 개발 인수인계 및 다른 PC 이전 가이드

이 문서는 Scaprobe 소스 코드를 다른 PC로 옮겨 현재 상태에서 개발을 이어가기 위한 기준 문서다. 문서 기준 버전은 `0.1.0`이며 마지막 갱신일은 2026-08-01이다.
...
- FastAPI REST API와 단일 파일 HTML/JavaScript 대시보드(`netprobe/static/dashboard.html`)
- 루프백이 아닌 바인드에 대한 API 토큰 인증(`--api-token`, `SCAPROBE_API_TOKEN`)
...
- Python 앱: `netprobe/version.py`
- Rust 엔진: `crates/scaprobe-engine/Cargo.toml`
...
Tauri 데스크톱 창
  -> 임의의 127.0.0.1 포트에서 PyInstaller 백엔드 실행
    -> FastAPI API 및 netprobe/static/dashboard.html 화면 제공
      -> Python이 입력 검증, 승인 범위, 작업 상태, SQLite를 담당
      -> scaprobe-engine Rust 프로세스가 실제 TCP/UDP 스캔 수행
...
| 경로 | 역할 |
| --- | --- |
| `netprobe/cli.py` | CLI 명령과 인자 정의 |
| `netprobe/api.py` | FastAPI 엔드포인트와 백그라운드 스캔 작업 |
| `netprobe/static/dashboard.html` | 별도 프런트엔드 프레임워크 없이 제공되는 대시보드 HTML/CSS/JavaScript 본문 |
| `netprobe/dashboard.py` | 위 정적 파일을 읽어 제공하는 로더 |
| `netprobe/auth.py` | 스캔 범위 승인과 API 토큰 결정 |
| `netprobe/engine.py` | Rust 엔진 탐색, 실행, NDJSON 이벤트 변환 및 취소 |
| `netprobe/storage.py` | SQLite 스키마와 저장소 계층 |
| `netprobe/evidence.py` | Playwright 웹 캡처와 터미널 증거 이미지 |
| `netprobe/config.py` | TOML 설정과 환경별 스캔 기본값 |
| `netprobe/plugins.py` | 실행 코드가 아닌 JSON 데이터 플러그인 검증/병합 |
| `netprobe/pcap.py` | PCAP/PCAPNG 분석 |
| `netprobe/packet_sender.py` | 제한된 패킷 템플릿 검증과 송신 |
| `crates/scaprobe-engine/src/main.rs` | 실제 TCP/UDP 스캔 및 서비스 지문 처리 |
| `crates/scaprobe-engine/tests/engine_cli.rs` | Rust 엔진 CLI 통합 테스트 |
| `desktop/src-tauri/src/main.rs` | 백엔드 프로세스 시작, 헬스체크, 창 생성 및 프로세스 종료 |
| `netprobe/frozen_backend.py` | PyInstaller 백엔드 진입점 |
| `tools/build_desktop.py` | 독립 실행형 Windows 설치 프로그램 전체 빌드 |
| `tools/package.py` | Python 기반 휴대용 ZIP/TAR 패키지 생성 |
| `tests/` | Python 단위·통합 테스트 |
| `docs/scaprobe.example.toml` | 설정 파일 예제 |
...
```text
C:\Users\<기존사용자>\AppData\Roaming\Scaprobe\scaprobe.db
```

새 PC에서 Scaprobe를 한 번 실행해 `%APPDATA%\Scaprobe` 폴더를 만든 후 앱을 종료하고 DB를 복사할 수 있다. 더 안전한 방법은 기존 PC의 대시보드 또는 `/v1/db/export`로 백업하고 새 PC의 `/v1/db/import`로 가져오는 것이다.
...
.\target\release\scaprobe-engine.exe --version
```

GNU 도구 체인을 명시하려면 다음과 같이 실행한다.

```powershell
cargo +stable-x86_64-pc-windows-gnu build --release -p scaprobe-engine
```
...
```powershell
scaprobe scan `
  --config .\docs\scaprobe.example.toml `
  --env local `
  --targets 127.0.0.1 `
  --ports 80,443 `
  --scope 127.0.0.0/8 `
...
```text
desktop\src-tauri\target\release\bundle\nsis\Scaprobe_0.1.0_x64-setup.exe
```
...
```text
dist\Scaprobe_0.1.0_x64-setup.exe
...
```powershell
$expected = (Get-Content .\Scaprobe_0.1.0_x64-setup.exe.sha256).Split(" ")[0]
$actual = (Get-FileHash .\Scaprobe_0.1.0_x64-setup.exe -Algorithm SHA256).Hash.ToLower()
...
- 데이터베이스 스키마나 백업 형식을 변경하면 기존 `scaprobe.db` import/upgrade 호환성을 반드시 테스트한다.
...
| --- | --- | --- |
| 스캔 동작/성능 | `crates/scaprobe-engine/src/main.rs`, `netprobe/engine.py` | `crates/scaprobe-engine/tests/engine_cli.rs`, `tests/test_engine.py` |
| API | `netprobe/api.py`, `netprobe/models.py` | `tests/test_api.py` |
| 대시보드 UI | `netprobe/static/dashboard.html` | `tests/test_dashboard.py`, `tests/test_api.py`의 dashboard 테스트 |
| DB/이력 | `netprobe/storage.py` | `tests/test_storage.py` |
| 증거/스크린샷 | `netprobe/evidence.py` | `tests/test_evidence.py`, `tests/test_api.py` |
| 설정/프로필 | `netprobe/config.py`, `netprobe/scan_inputs.py` | `tests/test_config.py`, `tests/test_scan_inputs.py` |
| 플러그인 | `netprobe/plugins.py` | `tests/test_plugins.py` |
| 보고서/내보내기 | `netprobe/reports.py`, `netprobe/exporters.py` | `tests/test_reports.py`, `tests/test_exporters.py` |
