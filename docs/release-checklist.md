...
Use this checklist before publishing a Scaprobe artifact.
...
- Run `scaprobe-engine --version` when an engine build is present.
- Confirm `scaprobe serve --check` reports `app_version` and, for engine artifacts, `rust_engine_version`.
...
cargo build --profile portable -p scaprobe-engine
```
...
- Run `scaprobe --help` from the extracted artifact.
- Run `scaprobe --version` from the extracted artifact.
- Run `scaprobe serve --check` from the extracted artifact.
- Open or request `/dashboard` from `scaprobe serve`.
...
- Run a config-based scan smoke with `--config docs/scaprobe.example.toml --env local`.
- Generate a stored scan report with `scaprobe report <scan_id> --format html`.
...
- Validate `docs/scaprobe.plugin.example.json`, list it with `scaprobe plugins list`, and run a plugin port-profile scan smoke.
