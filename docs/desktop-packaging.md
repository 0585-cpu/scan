...
The ordinary installer keeps Npcap optional: TCP connect scans and file PCAP analysis work without it. A SYN-enabled build includes the Npcap-linked engine but does not include the Npcap installer by default. The destination user installs current official Npcap directly. The engine delay-loads the Npcap library rather than linking it at load time, so it still starts and scans by connect on a machine that has no driver yet, and it looks for that library both on the loader's search path and in `System32\Npcap`, so either Npcap install mode works and **Install Npcap in WinPcap API-compatible Mode** is not required; diagnostics offer SYN only when the engine reports the feature *and* the driver is detected, and a SYN scan forced without it fails with a message naming Npcap instead of ending the process. A private personal NSIS build can still embed an explicitly supplied official Npcap installer when its licence and distribution boundary permit that use.
