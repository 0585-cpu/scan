...
```powershell
$artifact = "netroach-0.2.1-windows-amd64.zip"
...
```sh
sha256sum -c netroach-0.2.1-linux-x86_64.tar.gz.sha256
```
...
```powershell
Expand-Archive .\netroach-0.2.1-windows-amd64.zip -DestinationPath .\netroach
...
```sh
mkdir -p netroach
tar -xzf netroach-0.2.1-linux-x86_64.tar.gz -C netroach
