...
from netroach.frozen_backend import build_parser, main


class FrozenBackendTests(unittest.TestCase):
    def test_parser_requires_a_valid_engine_and_port(self):
        parser = build_parser()
        args = parser.parse_args(["--engine-path", "netroach-engine.exe", "--port", "49152"])

        self.assertEqual(args.engine_path, "netroach-engine.exe")
        self.assertEqual(args.port, 49152)
        with self.assertRaises(SystemExit):
            parser.parse_args(["--engine-path", "netroach-engine.exe", "--port", "0"])

    def test_main_sets_engine_path_and_starts_uvicorn(self):
        with tempfile.TemporaryDirectory() as tmp:
            engine = Path(tmp) / "netroach-engine.exe"
            ...
            with (
                ...
                patch("netroach.api.create_app", return_value=app) as create_app,
                ...
                self.assertEqual(os.environ["NETROACH_ENGINE"], str(engine.resolve()))
    ...
    def test_debug_log_level_enables_the_request_log(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            engine.write_bytes(b"engine")
            with (
                patch("netroach.api.create_app", return_value=object()),
                patch("uvicorn.run") as uvicorn_run,
            ):
    ...
    def test_main_rejects_missing_engine(self):
        with self.assertRaisesRegex(SystemExit, "bundled Rust engine was not found"):
            main(["--engine-path", "missing-netroach-engine.exe"])
